...
class MarkdownFileService {
    ...
    */
    ...
    //新建文章
    async createMarkdownFile(email, requestData) {
        const {
            title = " ",
            description = " ",
            tag1 = "null",
        ...
        return await markdownFile.create({
            email, title, description, tag1, tag2, tag3, audit, content
        });
    }
    ...
    async getMarkdownByKeyWord(key) {
        return await markdownFile.findAll({
            ...
            raw: true,
            // 控制查询字段
            attributes: ["id", "title", "description", "updatedAt", "tag1", "tag2", "tag3"]
        });
    }
